...
<div class="wrap-self">
	...
	<div class="content" id="manageGoods">
		<div class="add-good">
			<H4>添加商品：</H4>
			<div class="input-group self-add">
			  <span class="input-group-addon" id="basic-addon1">商品条码</span>
			  <input type="text" class="form-control" placeholder="Username" aria-describedby="basic-addon1" v-model.trim="productNo">
			</div>
			<div class="input-group self-add">  
			  <span class="input-group-addon" id="basic-addon2" >入库数量</span>
			  <input type="text" class="form-control" placeholder="Number" aria-describedby="basic-addon2" v-model.number="productTotal">
			</div>
			<div class="input-group self-add">  
			  <span class="input-group-addon">进货价￥</span>
			  <input type="text" class="form-control" aria-label="Amount (to the nearest dollar)" v-model.number="productPrice">
			  <span class="input-group-addon">.00</span>
			</div>
			<div class="clear"></div>
			<div class="input-group good-remark">
				<span class="input-group-addon">商品备注</span>
				<textarea class="form-control" rows="3" cols="20" v-model="productRemark"></textarea>
			</div>
			<button type="button" class="btn btn-default btn-lg active" @click="addProduct">添加</button>
		</div>
		<div class="clear"></div>
		<div class="table-grp">
			...
			<div class="table-list">
				<table class="table table-hover">
				  <thead>
				  	...
				  	<th>进价(￥)</th><th>数量</th><th>折扣</th><th>备注</th>
				  </thead>
				  <tbody class="good-list">
				  	<tr class="active-good" v-for="product, index in productList">
				  		...
				  		<td>{{product.price}}</td><td>{{product.number}}</td><td>{{product.discount}}</td><td>{{product.remark}}</td>
				  	</tr>
				  </tbody>
				</table>
			</div>
